...
(function(document, $, wordfind) {
    ...
    var WordFindGame = function() {
        ...
        var wordsearchPlugin_splittedWords;
        var wordsearchPlugin_distractors;
         ...
         */
        var drawWords = function(el, wordsearchPlugin_words) {

            var output = '<ul>';
            for (var i = 0, len = wordsearchPlugin_words.length; i < len; i++) {
                var word = wordsearchPlugin_words[i];
                output += '<li class="word ' + word + '"> <img alt=img src="http://placehold.it/100x100/>" class="wordImage"/> ' + word;
            }
            output += '</ul>';
        ...
        return {
             ...
             */
            create: function(wordsearchPlugin_words, splitted_words, puzzleEl, wordsEl, options, distractors) {
                wordsearchPlugin_splittedWords = splitted_words;
                wordsearchPlugin_letters = distractors;
                wordList = wordsearchPlugin_words.slice(0).sort();

                var puzzle = wordfind.newPuzzle(wordsearchPlugin_words, options, distractors);
            ...
            solve: function(puzzle, wordsearchPlugin_words, wordsearchPlugin_splittedWords) {
                ...
                var solution = wordfind.solve(puzzle, wordsearchPlugin_words, wordsearchPlugin_splittedWords).found;

                for (var i = 0, len = solution.length; i < len; i++) {
                    var word = solution[i].splittedWord,
